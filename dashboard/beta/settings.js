...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // settings.js - Update the loadUserData function
function loadUserData() {
    console.log('Loading user data for:', currentUser.uid);
    db.collection('users').doc(currentUser.uid).get()
        .then((doc) => {
            if (doc.exists) {
                ...
                //     userBadgeName.textContent = fullName || user.email.split('@')[0];
                // }
                
                // // Update user badge avatar if exists
                // const userBadgeAvatar = document.querySelector('.user-badge-avatar');
                // if (userBadgeAvatar) {
                //     userBadgeAvatar.src = userData.profileImage || 'avatars/default.png';
                // }
            } else {
                console.log('No user document found, creating default');
                return db.collection('users').doc(currentUser.uid).set({
