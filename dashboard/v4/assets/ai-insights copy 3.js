...
//   }
// =========================
// AI chat (Gemini or Ollama) with Web Search
// =========================
async function handleAsk(){
    const q = (questionInput?.value||'').trim();
    if (!q) return;
    ...
    try {
        ...
        const prompt = buildPrompt(q, s, userName, webResults);
        const mode = (window.AI_CONFIG?.MODE||'gemini').toLowerCase();
        let answer = '';
        if (mode==='gemini') answer = await callGemini(prompt);
        else answer = await callOllama(prompt);
...
// Update the SYSTEM_PROMPT in ai-config.js to be multilingual
...
// Now update the ai-insights.js file with multilingual support
async function handleAsk(){
    const q = (questionInput?.value||'').trim();
    if (!q) return;
    addMessage(q, 'user');
    ...
    try {
        ...
        let webResults = '';
        if (needsWebSearch) {
            webResults = await performWebSearch(q, detectedLanguage);
        }
        
        const prompt = buildPrompt(q, s, userName, webResults, detectedLanguage);
        const mode = (window.AI_CONFIG?.MODE||'gemini').toLowerCase();
        let answer = '';
        if (mode==='gemini') answer = await callGemini(prompt);
        else answer = await callOllama(prompt, detectedLanguage);
        removeTyping(typing);
        addMessage(answer||"Sorry, I couldn't generate a response.", 'bot');
    } catch (e){
        console.error(e);
...
// Function to detect language (simple implementation)
function detectLanguage(text) {
    ...
    // You can add more language detection here
    // For now, default to English
    return 'english';
}

// Update web search to include language context
async function performWebSearch(query, language) {
    try {
        // Add language context to search query
        let searchQuery = query + ' diabetes';
        if (language === 'arabic') {
            searchQuery += ' موقع:.sa OR موقع:.ae OR موقع:.eg'; // Saudi, UAE, Egypt domains
        } else {
            searchQuery += ' site:.gov OR site:.org OR site:.edu';
        }
        
        // Use a CORS proxy to avoid cross-origin issues
        const proxyUrl = 'https://cors-anywhere.herokuapp.com/';
        const searchUrl = `https://www.googleapis.com/customsearch/v1?key=YOUR_GOOGLE_SEARCH_API_KEY&cx=YOUR_SEARCH_ENGINE_ID&q=${encodeURIComponent(searchQuery)}`;
...
function buildPrompt(userQuestion, summary, userName, webResults = '', language = 'english') {
    ...
    if (language === 'arabic') {
        languageInstruction = 'RESPOND IN ARABIC. Use Arabic emojis and culturally appropriate expressions.';
    } else {
        languageInstruction = 'Respond in the same language as the user question.';
    }

    let task = '';
    
    if (webResults) {
        task = `The user is asking a question that might benefit from current information. Below are web search results. Use these to provide an up-to-date answer while still incorporating their personal data where relevant:\n\n${webResults}\n\n${languageInstruction} Please provide a helpful response based on both the user's data and the search results. Include a brief disclaimer if discussing medical information.`;
    } else if (medIntent) {
        task = `The user is asking about a medicine. ${languageInstruction} Provide:
...
Avoid dosing or individualized advice.`;
    } else {
        task = `Analyze the user's question using the data summary. ${languageInstruction} Mention concrete numbers and suggestions (hydration, walking, meal composition), but DO NOT give medical advice.`;
        // task = `Analyze the user's question using the data summary. ${languageInstruction} Mention concrete numbers and suggestions (hydration, walking, meal composition), but DO NOT give medical advice. Include a brief disclaimer.`;
    }

    return `${window.AI_CONFIG?.SYSTEM_PROMPT || ''}\n\n${summaryText}\nUser question: ${userQuestion}\n\n${task}`;
}

// Update the Ollama call to include language context
async function callOllama(prompt, language = 'english') {
    ...
    // For multilingual models, we can add language instructions
    let systemPrompt = window.AI_CONFIG?.SYSTEM_PROMPT || '';
    if (language === 'arabic') {
        systemPrompt += " Respond in Arabic with appropriate Arabic emojis and cultural sensitivity.";
    }
...
// Add RTL support for Arabic messages in the UI
...
function buildPrompt(userQuestion, summary, userName, webResults = '') {
    const medIntent = /medicin|medicine|drug|pill|tablet|insulin|metformin|sulfonylurea|glp|sglt|dpp|ozempic|semaglutide|tirzepatide|jardiance|metformin/i.test(userQuestion);
    const summaryText = `DATA SUMMARY\n- User Name: ${userName}\n- Count: ${summary.count}\n- Time-in-Range: ${summary.tirPct.toFixed(0)}% (70–180 mg/dL)\n- Avg: ${summary.avg.toFixed(0)}, SD: ${summary.sd.toFixed(0)}\n- Peak hour: ${summary.topHourLabel}\n- Post-meal Δ: ${summary.postMealDelta.toFixed(0)} (hotspot: ${summary.postMealHotspot})\n- Night avg: ${summary.nightAvg.toFixed(0)}, Morning avg: ${summary.morningAvg.toFixed(0)}\n- Weekday vs Weekend avg: ${summary.weekdayAvg.toFixed(0)} / ${summary.weekendAvg.toFixed(0)}\n`;
...
Avoid dosing or individualized advice.`;
        ...
        task = `Analyze ${userName}'s question using the data summary. Mention concrete numbers and suggestions (hydration, walking, meal composition), but DO NOT give medical advice. Include a brief disclaimer.`;
    }

    return `${window.AI_CONFIG?.SYSTEM_PROMPT || ''}\n\n${summaryText}\nUser question: ${userQuestion}\n\n${task}`;
}

// Alternative web search function using DuckDuckGo (no API key needed)
async function performDuckDuckGoSearch(query) {
    try {
        // Use a CORS proxy
        const proxyUrl = 'https://cors-anywhere.herokuapp.com/';
        const searchUrl = `https://html.duckduckgo.com/html/?q=${encodeURIComponent(query + ' diabetes')}`;
        ...
        if (results.length > 0) {
            results.slice(0, 3).forEach((result, index) => {
                const title = result.querySelector('.result__a')?.textContent || 'No title';
                const snippet = result.querySelector('.result__snippet')?.textContent || 'No snippet available';
