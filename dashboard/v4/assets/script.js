...
function exportAsPDF(readings, currentUser) {
    ...
    if (isRTL) {
        if (JannaBoldBase64.length < 200) { // Check if font data is actually present
            alert('Arabic font not provided. PDF text may be incorrect. Please add JannaBoldBase64 data.');
        } else {
            try {
                // Correctly add font to jsPDF instance
                pdf.addFileToVFS('JannaLTBold.ttf', JannaBoldBase64);
                pdf.addFont('JannaLTBold.ttf', 'Janna', 'normal'); // Use 'normal' for regular weight
                pdf.addFont('JannaLTBold.ttf', 'Janna', 'bold'); // Add bold variant if needed
                fontName = 'Janna';
            } catch (e) {
                console.error("Error loading Arabic font:", e);
                alert("Error loading Arabic font. PDF text may be incorrect.");
            }
        }
    }
    pdf.setFont(fontName); // Set the font for the document
    ...
    db.collection('users').doc(currentUser.uid).get()
        .then((docSnap) => {
           ...
           // Title
            ...
            // === Table ===
            const headers = ['Date', 'Time', 'Glucose (mg/dL)', 'Comment'];
            const columnWidths = [40, 30, 40, 80]; // Adjusted to match table width
            const rowHeight = 8;

            // Draw CONTINUOUS blue header row - key change is here
            pdf.setFont(fontName, 'bold');
            pdf.setFillColor(37, 99, 235); // Blue
            pdf.setTextColor(255, 255, 255); // White
            
            // Draw single continuous rectangle for header background
            pdf.rect(10, y, columnWidths.reduce((a, b) => a + b, 0), rowHeight, 'F');
            
            // Add header text (with RTL adjustments for last column if needed)
            let x = 10;
            ...
            readings.forEach((reading) => {
                ...
                if (y > 270) {
                    ...
                    // Redraw header on new page with continuous blue
                    pdf.setFont(fontName, 'bold');
...
if (exportPDFButton) {
    exportPDFButton.addEventListener('click', () => {
        if (currentUser) {
            exportAsPDF(readings, currentUser);
        } else {
            alert('Please sign in to export readings to PDF.');
        }
    });
}
...
// Create export to Excel button functionality
    ...
    db.collection('users').doc(currentUser.uid).get()
        .then((doc) => {
            ...
            const colWidths = [
                { wch: 12 }, // Date column
                { wch: 10 }, // Time column
                { wch: 15 }, // Glucose column
                { wch: 30 }  // Comment column
            ];
            worksheet['!cols'] = colWidths;
            
            // Add worksheet to workbook
            XLSX.utils.book_append_sheet(workbook, worksheet, 'Glucose Readings');
            ...
            const link = document.createElement('a');
            link.href = url;
            link.download = `glucose-readings-${new Date().toISOString().split('T')[0]}.xlsx`;
            link.click();
...
// Add this CSS for styling the Excel export button
