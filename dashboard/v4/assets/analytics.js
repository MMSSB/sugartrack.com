...
function exportDataReport() {
    ...
    // Format time properly for Excel (HH:MM format)
    const formatTimeForExport = (timestamp, timeStr) => {
        // Use the stored time string if available and properly formatted
        if (timeStr && /^\d{1,2}:\d{2}\s*(AM|PM)?$/i.test(timeStr)) {
            return timeStr;
        }
    ...
    readings.forEach(reading => {
        const formattedDate = formatDateForExcel(reading.timestamp);
        const formattedTime = formatTimeForExport(reading.timestamp, reading.time);
