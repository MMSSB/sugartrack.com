...
// Update readings list with proper error handling and loading states
function updateReadingsList() {
    console.log('Updating readings list with', readings.length, 'readings');
    
    // Show loading state
    readingsList.innerHTML = '<div class="reading-item loading">Loading readings...</div>';
    
    // Only proceed if we have readings
    if (readings.length === 0) {
            ...
            button.addEventListener('click', async (e) => {
                ...
                if (confirm('Are you sure you want to delete this reading?')) {
                    try {
                        ...
                        if (readingItem) {
                            readingItem.classList.add('deleting');
                            readingItem.innerHTML = '<div class="deleting-message">Deleting...</div>';
                        }
                        ...
                        alert('Error deleting reading. Please try again.');
                        if (readingItem) {
                            readingItem.classList.remove('deleting');
...
// Update the onSnapshot listener to handle errors better
window._unsubscribeReadings = db.collection('readings')
    .where('userId', '==', currentUser.uid)
    .orderBy('timestamp', 'desc')
    .onSnapshot((querySnapshot) => {
        ...
        console.error("Error in real-time updates:", error);
        readingsList.innerHTML = `<div class="error-message">Error loading readings. Please try refreshing the page.</div>`;
    });
// // Update readings list
// function updateReadingsList() {
//     readingsList.innerHTML = '';
...
function exportAsImage() {
    try {
        ...
        const height = margin * 2 + fontSizeTitle + heightPerRow * (sortedReadings.length + 1);
        const canvas = document.createElement('canvas');
        canvas.width = width;
        canvas.height = height;
        const ctx = canvas.getContext('2d');
        ...
        // Draw table headers
        const headers = ['Date', 'Time', 'Glucose (mg/dL)', 'Comment'];
        const columnWidths = [100, 80, 100, width - 300]; // Adjust as needed
        let yPosition = margin + fontSizeTitle + heightPerRow;
...
function exportAsPDF() {
    try {
        ...
        // Table setup
        const headers = ['Date', 'Time', 'Glucose', 'Comment'];
        const columnWidths = [40, 25, 25, 100]; // mm units
        const rowHeight = 7;
        let yPosition = 25;
